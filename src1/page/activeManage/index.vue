  ...
  <div class="activeManage ">
    <div class="app-container">
      ...
      <el-table  v-if="list" v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%">
        ...
        <el-table-column label="Actions" align="center" class-name="small-padding fixed-width">
          <template slot-scope="{row}">
            ...
            <!-- <el-button v-if="row.state!=2" size="mini" type="danger" @click="handleModifyStatus(row,'deleted')">
              删除
            </el-button> -->
          </template>
        </el-table-column>
      ...
      <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="75%">
        ...
        <!-- 图库 -->
        <div class="filter-container">
          <el-button class="filter-item" style="margin-bottom: 10px;" type="primary" icon="el-icon-edit" @click="addImg">
            添加
          </el-button>
        </div>
        <el-table v-loading="listLoading" :data="imgList" border fit highlight-current-row style="width: 100%">
          <el-table-column prop="houseId" label="名宿id" ></el-table-column>
          <el-table-column prop="id" label="房间id" ></el-table-column>
        ...
        <div slot="footer" class="dialog-footer">
          ...
          <el-button  v-if="dialogStatus=='update'" type="success" @click="preview(temp.id)">
            预览
          </el-button>
        </div>
      </el-dialog>
      ...
      <el-dialog title="预览" :visible.sync="dialogIframeVisible">
        ...
        <el-form ref="dataForm" :model="img" :rules="imgRules" label-position="left" label-width="100px" style="width: 80%; margin-left:50px;">
          ...
          <el-form-item label="图片地址" prop="img" class="total_box">
            ...
            >
              ...
              <div class="el-upload__tip" slot="tip">只能上传jpg/png文件，且宽为710px</div>
            </el-upload>
            <img :src="img.img" class="img_box">
          </el-form-item>
        </el-form>
        <div slot="footer" class="dialog-footer">
          <el-button @click="dialogFormVisible = false">
  ...
  data () {
    return {
      ...
      rules:{
        ...
        wash: [
          { type: 'number', message: '图片数量必须为数字值'},
  ...
  methods: {
    ...
    async submitImg (img) {
      ...
      for (const index in arrRules) {
        if (arrRules[index] == '' || arrRules[index] == [] || arrRules[index] == null || arrRules[index] == undefined) {
          console.log(index,this.roomRules[index])
          this.$message.error(this.imgRules[index][0]["message"])
          return
        }
      }
      console.log(img,'img')
